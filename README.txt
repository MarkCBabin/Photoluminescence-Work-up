...
PL_master_run is a script that will add the path of PL_master and Abs_avg to Matlab so that the files do not need to be copied into the same directory as the data being processed. It also impliments a graphic UI to open files, rather than from the standard folder system implimented in Matlab. 
